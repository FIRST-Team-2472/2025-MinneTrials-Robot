...
public class MotorPowerController {
    ...
    private double Kp, Ki, integral, dTime, error, timeIncrement, lastSensorRead, changeSpeed, predictionPoint,
            speedNeeded, allowedError, integralProportionalThreshold;
     ...
     * @param dTime                         Time into future the controller predicts
     *                                      to adjust speed
     * @param time                          Time it takes to get from 0 to 1
     * @param allowedError                  The error allowed before the integral
     *                                      starts to change
     * @param lastSensorRead                The last sensor read
     * @param integralProportionalThreshold The error threshold in which the
     *                                      integral starts changing by smaller
     *                                      increments
     */
    public MotorPowerController(double Kp, double Ki, double dTime, double time, double allowedError,
            double lastSensorRead, double integralProportionalThreshold) {
        this.dTime = dTime * 50; // 50 = code refresh rate (per second)
        this.integralProportionalThreshold = integralProportionalThreshold * 2; // multiplies by 2 for math to work i
                                                                                // forgot why
        ...
        error = 0; // starts at 0
        timeIncrement = 1 / time / 50; // 50 = code refresh rate (per second) this is the maximum amount the integral
                                       // can change per code frame
    }
    ...
    public double calculate(double setpoint, double sensorRead) {
        ...
        if (Math.abs(error) > allowedError) { // if error is greater than allowed error then integral is added
                                              // proportional to the error
            if (error < 0 && integral > -1)
                integral -= Math.min(timeIncrement,
        ...
        predictionPoint = (sensorRead - lastSensorRead) * dTime + sensorRead; // predicts position dTime seconds in the
                                                                              // future
        changeSpeed = (setpoint - predictionPoint) * Kp; // changes speed proportional to the error
        if (dTime == 0) { // if dTime is 0 then changeSpeed is 0 this corrects it doubling the
